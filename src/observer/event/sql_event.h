...
RC forwarding_insert(Trx *trx, const std::vector<Value> &values, Table *table);
...
public:
  ...
  QueryContext *get_query_context() const { return query_context_.get(); }
  QueryContext *release_query_context() { return query_context_.release(); }
...
private:
  ...
  std::unique_ptr<QueryContext>     query_context_;   ///< 查询上下文
};
