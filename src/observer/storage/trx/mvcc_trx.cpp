...
RC MvccTrx::insert_record(Table *table, Record &record)
{
  ...
  table->begin_xid_ = -trx_id_;
  table->end_xid_ = trx_kit_.max_trx_id();
  table->trx_ = this;
...
RC MvccTrx::delete_record(Table * table, Record &record)
{
  ...
  int index_offset = table->table_meta().sys_field_num() + table->table_meta().null_field_num();
  memcpy(&decode_index_num,record.data() + index_offset * 4,4);
  table->mvcc_unique_index_[decode_index_num].second = -trx_id_;
