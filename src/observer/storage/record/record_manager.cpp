...
RC   RecordPageHandler::init_empty_text_page(DiskBufferPool &buffer_pool, PageNum page_num, int record_size)
{
  RC ret = init(buffer_pool,page_num, false);
  page_header_->record_num = 0;
  page_header_->record_real_size = record_size;
  page_header_->first_record_offset = align8(PAGE_HEADER_SIZE);
  if((ret = buffer_pool.flush_page(*frame_)) !=RC::SUCCESS) {
    LOG_ERROR("Failed ot flush page header %d",page_num);
    return ret;
  }
  return RC::SUCCESS;
}
...
RC   RecordPageHandler::get_text_record(Record *record) {
  record->set_data(get_record_data(0),page_header_->record_real_size);
  return RC::SUCCESS;
}
...
RC RecordFileHandler::insert_text_record(const char *data,std::vector<PageNum > &text_page_nums) {
  int first_offset = align8(PAGE_HEADER_SIZE);
  int page_capacity = BP_PAGE_DATA_SIZE - first_offset;
  int data_len = static_cast<int>(strlen(data));
  if (data_len > 65535) {
    return RC::INVALID_ARGUMENT;
  }
  int page_num_in_need = data_len / page_capacity;
  int data_left_len = data_len % page_capacity;
  int offset = 0;
  RC ret = RC::SUCCESS;
  for (int i = 0;i < page_num_in_need;i++) {
    ret = insert_single_text_record(data + offset,page_capacity,text_page_nums);
    if (ret != RC::SUCCESS) {
      return  ret;
    }
    offset += page_capacity;
  }
  ret = insert_single_text_record(data + offset,data_left_len,text_page_nums);
  return ret;
}

RC RecordFileHandler::insert_single_text_record(const char *data, int len, vector<PageNum> &text_page_nums)
{
  RC ret = RC::SUCCESS;
  Frame *frame = nullptr;
  ret = disk_buffer_pool_->allocate_page(&frame);
  if (ret != RC::SUCCESS) {
    return ret;
  }

  RecordPageHandler record_page_handler;
  ret = record_page_handler.init_empty_text_page(*disk_buffer_pool_,frame->page_num(),len);
  if (ret != RC::SUCCESS) {
    return ret;
  }
  int first_offset = align8(PAGE_HEADER_SIZE);
  memcpy(frame->data() + first_offset,data,len);
  frame->mark_dirty();
  frame->unpin();
  text_page_nums.push_back(frame->page_num());
  disk_buffer_pool_->flush_page(*frame);
  return ret;
}
...
RC RecordFileHandler::get_text_record(PageNum page_num,Record *record,bool readonly) {
  RC ret = RC::SUCCESS;
  RecordPageHandler record_page_handler;
  ret = record_page_handler.init(*disk_buffer_pool_,page_num,readonly);
  if (OB_FAIL(ret)) {
    LOG_ERROR("Failed to init text record page");
    return ret;
  }
  return record_page_handler.get_text_record(record);
...
RC RecordFileHandler::delete_text_page(PageNum page_num,int record_size) {
  RecordPageHandler record_page_handler;
  free_pages_.insert(page_num);
  return record_page_handler.init_empty_page(*disk_buffer_pool_,page_num,record_size);
}
...
RC RecordFileScanner::fetch_next_record()
{
  ...
  while (bp_iterator_.has_next()) {
    ...
    if (table_->is_text_page(page_num)) {
      continue;
    }
