...
public:
  ...
  RC get_text_record(Record *record);
...
public:
  ...
  RC insert_text_record(const char *data,std::vector<PageNum >& text_page_nums);
  RC insert_single_text_record(const char *data,int len,std::vector<PageNum >&text_page_nums);

  RC get_text_record(PageNum page_num,Record *record,bool readonly);

  RC delete_text_page(PageNum page_num,int record_size);
