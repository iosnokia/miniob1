...
RC DefaultConditionFilter::init(Table &table, const ConditionSqlNode &condition)
{
  ...
  if (auto* field_expr = dynamic_cast<FieldExpr*>(condition.left.get())) {
    left.is_attr = true;
    const FieldMeta *field_left = table_meta.field(field_expr->node().attribute_name.c_str());
    if (nullptr == field_left) {
      LOG_WARN("No such field in condition. %s.%s", table.name(), field_expr->node().attribute_name.c_str());
  ...
  } else if (auto* value_expr = dynamic_cast<ValueExpr*>(condition.left.get())){
    left.is_attr = false;
    value_expr->get_value(left.value);  // 校验type 或者转换类型
    type_left = value_expr->value_type();

    left.attr_length = 0;
    left.attr_offset = 0;
  }

  if (auto* field_expr = dynamic_cast<FieldExpr*>(condition.right.get())) {
    right.is_attr = true;
    const FieldMeta *field_right = table_meta.field(field_expr->node().attribute_name.c_str());
  ...
  } else if (auto* value_expr = dynamic_cast<ValueExpr*>(condition.right.get())){
    right.is_attr = false;

    value_expr->get_value(right.value);  // 校验type 或者转换类型
    type_right = value_expr->value_type();
