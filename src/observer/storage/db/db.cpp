...
RC Db::create_table(const char *table_name, int attribute_count, const AttrInfoSqlNode *attributes, Table *&table)
{
  RC rc = RC::SUCCESS;
  // check table_name
...
RC Db::drop_table(const char *table_name)
{
  ...
  if (it == opened_tables_.end()) {
    LOG_ERROR("Failed to drop table,table: %s is not existed", table_name);
    return RC::SCHEMA_TABLE_NOT_EXIST;
  }
  Table *table = it->second;
  auto   res   = table->drop();
  if (res != RC::SUCCESS) {
    LOG_ERROR("Failed to drop table and remove file");
    return res;
  }
