...
FieldMeta::FieldMeta(const std::string& name, AttrType attr_type, int attr_offset, int attr_len, bool visible,bool is_allow_null)
{
  [[maybe_unused]] RC rc = this->init(name, attr_type, attr_offset, attr_len, visible,is_allow_null);
  ASSERT(rc == RC::SUCCESS, "failed to init field meta. rc=%s", strrc(rc));
}


RC FieldMeta::init(const std::string& name, AttrType attr_type, int attr_offset, int attr_len, bool visible,bool is_allow_null)
{
  ...
  if (AttrType::UNDEFINED == attr_type || attr_offset < 0 || attr_len <= 0) {
    LOG_WARN(
        "Invalid argument. name=%s, attr_type=%d, attr_offset=%d, attr_len=%d", name.c_str(), attr_type, attr_offset, attr_len);
    return RC::INVALID_ARGUMENT;
  }
  ...
  visible_ = visible;
  is_allow_null_ = is_allow_null;

  LOG_INFO("Init a field with name=%s", name.c_str());
  return RC::SUCCESS;
}
...
bool FieldMeta::is_allow_null() const
{
...
void FieldMeta::desc(std::ostream &os) const
{
  os << "field name=" << name_ << ", type=" << attr_type_to_string(attr_type_) << ", len=" << attr_len_
     << ", visible=" << (visible_ ? "yes" : "no") <<",is allow null=" << (is_allow_null_? "yes" : "no");
}
...
void FieldMeta::to_json(Json::Value &json_value) const
{
  ...
  json_value[FIELD_NULL] = is_allow_null_;
}
...
RC FieldMeta::from_json(const Json::Value &json_value, FieldMeta &field)
{
  ...
  if (!null_value.isBool()) {
    LOG_ERROR("null field is not a bool value,joins value=%s",null_value.toStyledString().c_str());
    return RC::INTERNAL;
  }
  ...
  bool is_allow_null = null_value.asBool();
  return field.init(name, type, offset, len, visible,is_allow_null);
}
