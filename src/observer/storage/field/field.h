...
public:
  ...
  Field(const Table *table, const FieldMeta *field, bool sort_way) : table_(table), field_(field), sort_way_(sort_way)
  {}

  const Table     *table() const { return table_; }
  const FieldMeta *meta() const { return field_; }
  const bool       get_sort_way() const { return sort_way_; }
  bool            &sort_way() { return sort_way_; }
