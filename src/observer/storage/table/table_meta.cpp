...
RC TableMeta::init(int32_t table_id, const char *name, int field_num, const AttrInfoSqlNode attributes[])
{
  ...
  if (trx_fields != nullptr) {
    ...
    // 创建sys field，但是似乎目前没有任何的sys_field
  ...
  // 先添加一个null field，null field使用4个字节的chars存储即可，代表一个32位的bitmap
  fields_[trx_field_num].init(
      "null_field",
      AttrType::CHARS,
      field_offset,
      4,
  ...
  for (int i = 0; i < field_num; i++) {
    ...
    rc = fields_[i + trx_field_num + null_field_num()].init(attr_info.name.c_str(),
            attr_info.type, field_offset, attr_info.length, true/*visible*/,attr_info.is_allow_null);
    if (rc != RC::SUCCESS) {
