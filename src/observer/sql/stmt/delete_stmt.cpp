...
{
  const char *raw_table_name = delete_sql.relation_name.c_str();
  ...
  let         sql_node    = container_of(&delete_sql, ParsedSqlNode, deletion);
  RC          rc          = FilterStmt::create(db, table, &table_map, delete_sql.conditions.get(), sql_node, root);
  let         filter_stmt = new FilterStmt(std::unique_ptr<FilterTree>(root));

  //  RC rc = FilterStmt::create(
  //      db, table, &table_map, delete_sql.conditions.get(), static_cast<int>(delete_sql.conditions.size()),
  //      filter_stmt);
  if (rc != RC::SUCCESS) {
