...
RC CreateIndexStmt::create(Db *db, const CreateIndexSqlNode &create_index, Stmt *&stmt)
{
  ...
  for(string s  : create_index.attribute_name){
     if(is_blank(s.c_str())){
        LOG_WARN("bad index attr");
        return RC::INVALID_ARGUMENT;
     }
  }
  ...
  for(string s  : create_index.attribute_name){
     if(table->table_meta().field(s.c_str()) != nullptr){
        field_meta->emplace_back(*(table->table_meta().field(s.c_str())));
     }
  }


  if (field_meta->size() != create_index.attribute_name.size()) {
