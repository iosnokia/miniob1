...
// 像是Insert语句，也会走这个函数，当时并不会在其中执行（没有对应的switch分支）
RC ExecuteStage::handle_request_with_physical_operator(SQLStageEvent *sql_event)
{
  ...
  switch (stmt->type()) {
    case StmtType::SELECT: {
      ...
      auto       *project         = static_cast<ProjectPhysicalOperator *>(physical_operator.get());
      bool        with_table_name = select_stmt->tables().size() > 1;
      if(select_stmt->order_by_elems().size()!=0){
        project = dynamic_cast<ProjectPhysicalOperator *>(project->children().back().get());
      }
      for (const auto &expr : project->expressions()) {
        if (!expr->name().empty() && dynamic_cast<FieldExpr*>(expr.get()) == nullptr) {
          schema.append_cell(expr->name().c_str());
        } else {
          FieldExpr *field_expr;
          if ((field_expr = dynamic_cast<FieldExpr *>(expr.get()))) {
            if (with_table_name) {
              schema.append_cell(field_expr->raw_table_name(), field_expr->field_name());
            } else {
