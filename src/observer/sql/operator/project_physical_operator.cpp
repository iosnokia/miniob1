...
RC ProjectPhysicalOperator::next()
{
  ...
  if (rc == RC::SUCCESS)
  {
    ...
    for (const auto &expr : expressions_) {
      Value val;
      let eval_rc = expr->get_value(*tuple, val);
    ...
    value_list_tuple_.set_cells(values);
  }
  return rc;
}
...
void ProjectPhysicalOperator::add_projection(const Table *table, const char *alias)
{
  TupleCellSpec *spec = new TupleCellSpec(table->name(), "", alias);
