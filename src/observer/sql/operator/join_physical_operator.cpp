...
RC NestedLoopJoinPhysicalOperator::next()
{
  while (true) {
    ...
    RC rc;
    while ((rc = right_next()) == RC::SUCCESS) {
      if (matched()) {
        return RC::SUCCESS;
      }
    }
...
//    rc = right_next();
//  }
//  //LOG_ERROR("current tuple:%s",joined_tuple_.to_string().c_str());
//
//
//  return rc;
...
//    joined_tuple_.set_right(right_tuple_);
//    Value value;
//    rc = expression_->get_value(joined_tuple_,value);
//    if (rc != RC::SUCCESS) {
//      return rc;
//    }
...
bool NestedLoopJoinPhysicalOperator::matched()
{
  RC rc = RC::SUCCESS;
  Value value;
