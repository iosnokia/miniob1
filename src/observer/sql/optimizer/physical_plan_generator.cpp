...
RC PhysicalPlanGenerator::create_plan(ProjectLogicalOperator &project_oper, unique_ptr<PhysicalOperator> &oper)
{
  ...
  const auto              &project_exprs    = project_oper.expressions();
  const FieldExpr         *field_expr;
  const AggregateExpr     *aggr_expr;
  //  for (const auto &expr : project_exprs) {
  //    switch (expr->type()) {
  //      case ExprType::NONE: break;
  //      case ExprType::STAR: break;
  //      case ExprType::FIELD:
  //        field_expr = dynamic_cast<const FieldExpr *>(expr.get());
  //        project_operator->add_projection(field_expr->table(), field_expr->field_meta());
  //        break;
  //      case ExprType::VALUE:
  //      case ExprType::CAST:
  //      case ExprType::COMPARISON:
  //      case ExprType::CONJUNCTION:
  //      case ExprType::ARITHMETIC: project_operator->add_projection(nullptr, expr->name().c_str());
  //      case ExprType::AGGREGATE:
  //        aggr_expr = dynamic_cast<const AggregateExpr *>(expr.get());
  //        project_operator->add_projection(aggr_expr->table(), aggr_expr->name().c_str());
  //        break;
  //    }
  //    //    project_operator->add_projection(field.table(), field.meta());
  //  }

  project_operator->inject_project_exprs(project_oper.take_exprs());
...
RC PhysicalPlanGenerator::create_plan(UpdateLogicalOperator &update_oper, unique_ptr<PhysicalOperator> &oper){
  ...
  let update_physical_oper = new UpdatePhysicalOperator(update_oper.table(),update_oper.field_and_value());
  update_physical_oper->fragile = update_oper.fragile;
  oper = unique_ptr<PhysicalOperator>(update_physical_oper);
...
RC PhysicalPlanGenerator::create_plan(JoinLogicalOperator &join_oper, unique_ptr<PhysicalOperator> &oper)
{
  ...
  unique_ptr<PhysicalOperator> join_physical_oper(new NestedLoopJoinPhysicalOperator(join_oper.expression()));
...
RC PhysicalPlanGenerator::create_plan(AggregateLogicalOperator &logical_oper, unique_ptr<PhysicalOperator> &oper)
{
  RC rc = RC::SUCCESS;
  if (!logical_oper.children().empty()) {
    create(*logical_oper.children().front(), oper);
  }
  auto *agg_oper = new AggregatePhysicalOperator(std::move(oper), logical_oper);
  oper.reset(agg_oper);
  return rc;
}
...
RC PhysicalPlanGenerator::create_plan(SortLogicalOperator &sort_oper, unique_ptr<PhysicalOperator> &oper)
{
  ...
  SortPhysicalOperator *sort_operator = new SortPhysicalOperator(sort_oper.fields());
