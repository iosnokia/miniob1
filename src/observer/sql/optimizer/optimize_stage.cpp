...
RC OptimizeStage::handle_request(SQLStageEvent *sql_event)
{
  ...
  rc = generate_physical_plan(sql_event, logical_operator, physical_operator);
  if (rc != RC::SUCCESS) {
...
RC OptimizeStage::generate_physical_plan(SQLStageEvent *sql_event, unique_ptr<LogicalOperator> &logical_operator,
    unique_ptr<PhysicalOperator> &physical_operator)
{
  RC rc = RC::SUCCESS;
  rc    = physical_plan_generator_.create(*logical_operator, physical_operator);
  physical_operator->set_query_context(sql_event->get_query_context());
  if (rc != RC::SUCCESS) {
