...
repeated run is reported. When `report aggregate_values only` option is `true`,
only the aggregate_values (i.e. mean, median, standard deviation and coefficient
...
However when only the `display aggregate_values only` option is `true`,
only the aggregate_values are displayed in the standard output, while the file
...
While having these aggregate_values is nice, this may not be enough for everyone.
